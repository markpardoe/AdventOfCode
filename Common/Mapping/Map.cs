...
namespace AoC.Common.Mapping
{
    ...
    public class Map<TValue> : BufferedMap<Position, TValue>
    {
        ...
        // Unlike _map.GetEnumerator this Will return all positions within bounds - even if they are not keys in the map collection
        // This allows us to store sparse maps - where the positions of default values / empty spaces don't need to be stored.
        public override IEnumerable<KeyValuePair<Position, TValue>> GetBoundedEnumerator(int padding = 0)
        {
            ...
            {
                yield return new KeyValuePair<Position, TValue>(position, this[position]);
            }
        }
        ...
        /// Relies on virtual method [GetNeighboringPositions] to filter the neighboring positions before adding them to the search list.
        /// By default this returns the 4 points in the 4 cardinal directions (not diagonal)
        /// </summary>
        /// <param name="start"></param>
        /// <param name="value"></param>
        /// <returns></returns>
        public MapNode FindPathToNearestValue(Position start, TValue value)
        {
            var targets = Map.Where(x => x.Value.Equals(value));
            return FindPathsToAllPositions(start, targets.Select(x => x.Key)).FirstOrDefault();
        }
        ...
        /// <returns></returns>
        public IEnumerable<MapNode> FindPathsToAllPositions(Position start, IEnumerable<Position> targets)
        {
            ...
            while (openList.Count > 0)
            {
                ...
                var neighbors = GetNeighboringPositions(currentPosition.Position)
                    .Select(x => new MapNode(x, currentPosition, currentPosition.DistanceFromStart + 1));

                foreach (var n in neighbors)
                {
                    // We've reached a target - BFS guarentees its the shortest path so return it
                    if (targetList.Contains(currentPosition.Position))
                    {
                        yield return currentPosition;

                        // We've found shortest-path to this position, so drop it.
                        // Also, allows us to optimize by stopping the search once all targets are found
        ...
        /// <param name="start"></param>
        /// <returns></returns>
        public IEnumerable<MapNode> FindPathToAllValues(Position start, TValue value)
        {
            var targets = Map.Where(x => x.Value.Equals(value));
            
            // Might be faster to use A* to find each position individually?
            return FindPathsToAllPositions(start, targets.Select(x => x.Key));
        }
        ...
        /// Relies on virtual method [GetNeighboringPositions] to filter the neighboring positions before adding them to the search list.
        /// By default this returns the 4 points in the 4 cardinal directions (not diagonal), but child classes can implement their own logic if required
        /// </summary>
        /// <param name="start"></param>
        /// <param name="target"></param>
        public MapNode FindPathToPosition(Position start, Position target)
        {
        ...
        // Calculating them is expensive on large maps - so we don't want to recalculate more than needed
