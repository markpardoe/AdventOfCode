...
{ 
    public class Day07Tests : AocSolutionTest<long>
    {
        ...
        public void Test_InCodeParse_RunAmplifiedCode(int expectedResult, int s1, int s2, int s3, int s4, int s5, params int[] code)
        {
            var settings = new List<long>() { s1, s2, s3, s4, s5 };
            AmplificationController controller = new AmplificationController(settings, code.Select(a => (long)a));
        ...
        public void Test_FeedbackAmplificationController(int expectedResult, int s1, int s2, int s3, int s4, int s5, params int[] code)
        {
            var settings = new List<long>() { s1, s2, s3, s4, s5 };
            FeedbackAmplificationController controller = new FeedbackAmplificationController(settings, code.Select(a => (long) a));
